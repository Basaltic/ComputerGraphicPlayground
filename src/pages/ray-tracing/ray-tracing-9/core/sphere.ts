...
export class Sphere implements Hittable {
  ...
  boundingBox(time0: number, time1: number): AABB {
    const outputBox = new AABB(
      this.center.sub(new Vector3(this.radius, this.radius, this.radius)),
...
export class MovingSphere implements Hittable {
  ...
  center(time: number) {
    ...
    const cc = this.center1.sub(this.center0).multiply(timex);
    return this.center0.add(cc);
  }

  hit(ray: Ray, tMin: number, tMax: number): HitRecord | undefined {
    const oc = ray.origin.sub(this.center(ray.time));
    ...
    hitRecord.t = root;
    hitRecord.p = ray.at(hitRecord.t);
    const outwardNormal = hitRecord.p.sub(this.center(ray.time)).divide(this.radius);
    hitRecord.setFaceNormal(ray, outwardNormal);
    hitRecord.materialPtr = this.materialPtr;
  ...
  boundingBox(time0: number, time1: number): AABB {
    const box0 = new AABB(
      this.center(time0).sub(new Vector3(this.radius, this.radius, this.radius)),
      this.center(time0).add(new Vector3(this.radius, this.radius, this.radius))
    );

    const box1 = new AABB(
      this.center(time1).sub(new Vector3(this.radius, this.radius, this.radius)),
