...
export abstract class Material {
   ...
   * @param scattered
   */
  abstract scatter(rayIn: Ray, hitRecord: HitRecord): [boolean, RGBColor, Ray];

  emitted(u: number, v: number, p: Vector3): Vector3 {
...
export class Dielectric extends Material {
  ...
  scatter(rayIn: Ray, hitRecord: HitRecord): [boolean, RGBColor, Ray] {
    ...
    const scattered = new Ray(hitRecord.p, dir, rayIn.time);
...
export class DiffuseLight extends Material {
  ...
  scatter(rayIn: Ray, hitRecord: HitRecord): [boolean, RGBColor, Ray] {
    return [false, new RGBColor(0, 0, 0), rayIn];
  }
